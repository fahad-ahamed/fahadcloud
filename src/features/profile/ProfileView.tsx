      ...
      <Card className="bg-white border-slate-200 shadow-sm">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {!profileEditing ? (
              <Button variant="outline" size="sm" className="border-slate-300" onClick={() => setProfileEditing(true)}>
                Edit
              </Button>
            ) : (
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.firstName} onChange={e => setProfileForm(p => ({ ...p, firstName: e.target.value }))} disabled={!profileEditing} />
            </div>
            <div>
              <Label className="text-slate-600">Last Name</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.lastName} onChange={e => setProfileForm(p => ({ ...p, lastName: e.target.value }))} disabled={!profileEditing} />
            </div>
            <div>
              <Label className="text-slate-600">Phone</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.phone} onChange={e => setProfileForm(p => ({ ...p, phone: e.target.value }))} disabled={!profileEditing} placeholder="+880 1XXXXXXXXX" />
            </div>
            <div>
              <Label className="text-slate-600">Company</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.company} onChange={e => setProfileForm(p => ({ ...p, company: e.target.value }))} disabled={!profileEditing} placeholder="Your company name" />
            </div>
            <div>
              <Label className="text-slate-600">Address</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.address} onChange={e => setProfileForm(p => ({ ...p, address: e.target.value }))} disabled={!profileEditing} placeholder="Street address" />
            </div>
            <div>
              <Label className="text-slate-600">City</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.city} onChange={e => setProfileForm(p => ({ ...p, city: e.target.value }))} disabled={!profileEditing} placeholder="City" />
            </div>
            <div className="sm:col-span-2">
              <Label className="text-slate-600">Country</Label>
              <Input className="bg-white border-slate-300 text-slate-900" value={profileForm.country} onChange={e => setProfileForm(p => ({ ...p, country: e.target.value }))} disabled={!profileEditing} placeholder="Country" />
            </div>
          </div>
          {profileEditing && (
      ...
      <Card className="bg-white border-slate-200 shadow-sm">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {!showChangePassword && (
              <Button variant="outline" size="sm" className="border-slate-300" onClick={() => setShowChangePassword(true)}>
                Change
              </Button>
            )}
          </div>
        ...
        {showChangePassword && (
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              {actionVerifyStep === 'verify' && actionVerifyAction === 'password_change' ? (
                <>
                  <Button className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700" onClick={onChangePassword} disabled={actionVerifyLoading}>
                ...
                <>
                  ...
                  <Button variant="outline" className="border-slate-300" onClick={() => { setShowChangePassword(false); setPasswordForm({ currentPassword: '', newPassword: '', confirmPassword: '' }); setActionVerifyStep('idle'); setActionVerifyOtp(''); }}>Cancel</Button>
                </>
              )}
            </div>
          </CardContent>
      ...
      <Card className="bg-white border-slate-200 shadow-sm">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" className="border-red-200 text-red-500 hover:bg-red-50 hover:text-red-600" onClick={onLogout}>
      ...
      <Card className="bg-white border-red-200 shadow-sm">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" className="border-red-300 text-red-600 hover:bg-red-50 hover:text-red-700" onClick={onDeleteAccount}>
